...
/plugin install dddart@changja88-dddart
```
...
codex plugin add dddart@changja88-dddart
```
...
```
/plugin marketplace update changja88-dddart
/plugin update dddart@changja88-dddart
```

**Codex** — CLI로. Codex에는 별도 `plugin update` 커맨드가 없다 — 설치된 플러그인이 마켓플레이스 스냅샷 디렉터리에서 직접 로드되므로, **스냅샷을 갱신하면 그게 곧 플러그인 업데이트**다:

```
codex plugin marketplace upgrade changja88-dddart
```
...
dddart@changja88-dddart   installed, enabled  1.1.0    ~/.codex/.tmp/marketplaces/changja88-dddart/codex-dddart
```
...
- **G2 · 구현** — 구현 코드 + 테스트 통과 결과 + 감수 리포트 + **60종 결정적 백스톱** 통과를 승인한다.

> G2 직전에는 **60종의 결정적 백스톱**(Dart 검사 스크립트)이 자동으로 돌아 구조·계약 회귀를 차단한다 — 컨테이너 위치, 4계층 골격, import 방향, 명명 규약, freezed 모델, @riverpod provider 위치 등을 고정밀로 검사해, 에이전트의 의미 감수가 놓칠 수 있는 위반을 마지막 안전망으로 잡는다.
...
**3) 구현 → G2**
coder가 domain부터 presentation까지 bottom-up으로 슬라이스를 구현하고, 각 계층 끝에서 `flutter analyze`·`flutter test`로 green을 확인한다(행위 테스트 포함). discipline-reviewer가 구조·규율을 감수하고, 60종 백스톱이 G2 직전에 자동으로 돈다. → 코드·테스트·감수·백스톱 결과를 당신이 승인.
...
핵심 규약이 일관되게 강제된다 — `application/` BC 컨테이너, 4계층 물리 분리, **개념 1차·종류 2차** 폴더, 도메인은 순수 Dart, ViewModel은 `@riverpod` 클래스형(UseCase·Repo는 plain class·DI 없음), State는 freezed, 시각 값은 design_system 토큰 단일 출처, View는 수동(watch·바인딩). **이 규약들은 60종의 결정적 백스톱이 구현 게이트(G2) 직전에 자동 검증**한다.
...
- **결정적 백스톱 60종**: 구조·계약 회귀를 G2 직전에 자동 차단하는 Dart 검사 스크립트
