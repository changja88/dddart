...
- (있으면) `design-ref/` 경로 — 화면 디자인 이미지·동결 화면 시안(JSX `screens/*Screen.jsx` 또는 Claude Design PROJECT의 `.dc.html`). "무엇처럼 보이나"의 단일 근거이되 디자인 엔진이 생성한 JSX(React)·`.dc.html`(HTML/CSS)는 직수입하지 않는다 — 토큰·아이콘·이미지 src·형상만 추출하고 위젯은 Flutter로 새로 쓴다(JSX·`.dc.html` 직수입 유혹↑ → 토큰·IR만).
- (있으면) `design-tokens.json` 경로 — Coordinator가 `_ds_manifest.json`의 `tokens[]={name,value,kind(color|font|spacing|radius|shadow|other)}`를 정규식 없이 **JSON 직독**해 kind별로 절단한 색·타이포·spacing·모서리·임의값 토큰 + 화면 JSX `window.BrkIcon` 스캔 아이콘(정확 Material Symbol명+`fill`+`Icons.*` 후보). 시각 명세의 단일 근거다 — 시안 색·spacing·아이콘을 네가 눈대중으로 추정하지 말고 이 토큰을 인용해 박는다. **kind→foundation 매핑**: color→`app_color`·font→`app_typography`·spacing→`app_spacing`·radius→`app_radius`·shadow→`app_shadow`(foundation 7토큰 중 5종 — houserules). **단 `arbitraryValues`에는 shadow 토큰명만 온다**(`--from-ds-manifest`가 shadow kind를 `arbitraryValues`로 라우팅); 치수(px)는 `typography`·`spacing`·`borderRadius` 버킷이다(설계 §5 kind 매핑). `--shadow-` 접두 토큰명은 `app_shadow` foundation으로 라우팅하고(§8 크기 전수 대상 아님); **`--shadow-` 등 `var(--shadow-…)` 토큰의 *값*은 design-tokens.json이 아니라 동결 `tokens/*.css`의 `var(--shadow-…)` 정의에서 해소한다**(extract_design이 shadow는 name만 싣고 값은 폐기).
- (있으면) `asset-manifest.json` 경로(`has_design_images`) — 화면 JSX(`*Screen.jsx`)의 `<img src>`(상대경로)→다운로드 `local_path`→`AppAsset.<token>` 매핑(단일 SSOT·fetch_images가 `--asset-base`로 상대경로를 해소해 산출). **너는 이 manifest를 읽어 *어느 이미지(src)가 어느 화면 조각에* 들어가는지 의미를 연결한다 — `token`·경로 문자열은 박지 않는다**(coder가 manifest에서 src로 직접 조인). design-tokens가 "무슨 색"이면 asset-manifest는 "어느 자리에 어느 이미지"다.
- 설계 명세를 저장할 경로.
...
- (있으면) 스코프가 전달한 **BC 그루핑(area) 판정** — 접두→`area | not-area`(G0 사용자 판정 — 판별 절차는 houserules undecidable.md §13). `area` 판정 접두의 새 BC는 파일 목록 경로를 `application/<area>/<bc>/…`로 적고, 판정이 없거나 `not-area`면 평면 `application/<bc>/…`로 적는다. **area를 네 판단으로 발명·생략하지 않는다** — area는 순수 시각 네임스페이스라 BC 경계·애그리거트·교차 BC 채널 설계에 아무 영향이 없고(houserules final.md §1 area 핵심 사실), 어떤 식별자·클래스명·라우트 name에도 등장시키지 않는다.
...
명세를 쓰기 전에 기존 프로젝트를 Read/Grep/Glob로 조사한다: ① 소스 디렉터리 구조(따라야 할 기존 레이아웃 규약 — 구조 결정의 근거) ② **`design_system/`과 `common/`의 재사용 후보**(이번 화면이 쓸 토큰·공용 위젯·기존 유틸 — 재사용 누락은 ui 리뷰어가 잡지만 1차 조사는 네 의무다. **화면 JSX가 `window.DesignSystem_<ns>`에서 조립한 컴포넌트(manifest `components[]={name,sourcePath}`)는 어느 `design_system/component/` 위젯을 재사용해야 하는지의 1차 신호다**) ③ 기존 BC를 확장하는 기능이면 그 BC의 현재 트리.
...
- **화면(ui)**: 화면 분해 — view(라우트 단위)·section(VM이 필요한 화면 전속 하위)·widget(수동 부품)의 판별과 *왜*, design_system 재사용 항목(조사 결과 — 새 토큰·공용 위젯이 필요하면 그 결정), 내비게이션 흐름(GoRoute 경로·branch 소속·진입/복귀), view 수동성(판단·가공이 VM으로 갔는지). **`design-tokens.json`이 있으면 그 색·spacing·크기(`typography`·`spacing`·`borderRadius` 버킷)·아이콘을 명세 화면 절에 박는다** — `colors`(kind=color)의 신규 색은 design_system foundation `app_color` 토큰 추가로 결정하고, 아이콘은 도메인 enum→`Icons.*` 매핑을 ui_extension에 둔다(architecture-ui §5 — `icons[]`의 `name`/`fill`[화면 JSX `window.BrkIcon`의 `name`·`fill={…}`에서 추출]/`flutter` 후보 인용·`unmappedIcons`는 가장 가까운 `Icons.*` + design-ref 충실도 확인). **`arbitraryValues`의 그림자(`--shadow-` 접두 토큰명·kind=shadow)는 `app_shadow` foundation으로 라우팅한다 — 크기 정형 목록 대상이 아니다.** **나머지 크기는 비도메인 `typography`·`spacing`·`borderRadius` 버킷의 치수 항목을 빠짐없이 1건씩 — 어느 조각의 어느 size/fontSize prop에 연결했는지(또는 왜 안 쓰는지) — 정형 목록으로 박는다**(추출 크기 토큰 수만큼 항목·빈칸이면 coder가 그 크기를 흘린다): 발명한 픽셀이 아니라 추출값 인용이며(출처는 화면 JSX inline-style의 수치 prop — **`.dc.html` 시안이면 인라인 `<style>`의 CSS 규칙을 class→규칙 조인으로 인용하며, 매니페스트 밖 화면 고유 px이고 토큰 매핑이 완전 직접은 아님[class→CSS indirection]**) 단일 사용처는 직접 인용·여러 조각 공유는 foundation 토큰 승격, 도메인 `typography`(본문·강조 텍스트 등)는 ui_extension 담당이라 제외다 — 단 *비-도메인 추출 typography 크기*(JSX inline-style `fontSize` 등 비도메인 글자 크기)는 ui_extension 매핑 대상이 아니므로 `app_typography` 토큰 정의·참조로 명세에 박는다(기존 토큰 위 `copyWith(fontSize: N)` 덮기로 명세하지 않는다)·비-typography 크기(`width`·아이콘 size)만 직접 인용이다(architecture-ui §8). **화면이 실제 조립하는 design_system 위젯 매핑도 박는다 — `_ds_manifest.json`의 `components[]={name,sourcePath}`와 화면 JSX가 `window.DesignSystem_<ns>`에서 조립한 컴포넌트(`<Card><Badge>` 등)를 근거로 '이 화면 조각 → 어느 `design_system/component/` 위젯'을 명시 매핑한다**(YAGNI 기본 — 화면이 실제 쓰는 컴포넌트만·전체 라이브러리 일괄 아님). **이미지도 같은 형식으로 — `asset-manifest.json`이 있으면(`has_design_images`) 각 `images` 항목(src 기준)을 빠짐없이 1건씩 — 어느 화면의 어느 조각이 그 이미지를 렌더하는지(또는 왜 안 쓰는지) — `src`로 가리켜 정형 목록으로 박는다**(manifest 항목 수만큼·빈칸이면 coder가 그 이미지를 흘린다). `AppAsset.<token>`·`assets/…` 경로 문자열은 박지 않는다 — coder가 manifest에서 `src`로 조인해 정확 값을 가져온다(architecture-ui §7 사용·§8과 동형: 도메인 없는 추출값을 명세에서 조각에 잇기·여기선 크기가 아니라 이미지). 레이아웃 형상(배치·축)은 명세에 적지 않는다 — coder가 동결 화면 시안(`*Screen.jsx` 또는 `.dc.html`)에서 재현한다(형상-SoT 규율 불변 — feedback-015 공리: 형상 출처가 `.jsx`+`.dc.html`로 합류할 뿐, architect 산문 레이아웃 기술은 여전히 금지). **`.dc.html` 시안의 형상 재현 대상은 앱 콘텐츠(`.screen`/`.body` 내부)만이다 — device-chrome(`.phone`/`.statusbar`/`.blob` 폰 목업)은 앱이 아니라 형상 재현에서 제외한다**(architecture-ui §8·implementation-flutter §9). 너는 분해·토큰·이미지·컴포넌트·내비까지.
...
초안·수정을 완료로 넘기기 전, 절 간 **자기모순을 1회 스캔**한다 — 같은 개념의 소유권(어느 계층·어느 객체가 판정을 갖는지)·명명·시그니처·불변식이 절마다 일치하는지, **구조 결정 절(파일 목록·분할) 포함** — 같은 개념이 두 철자로 적히거나 "두 번째 개념"의 분할 누락이 없는지(파일 목록 소유자는 너다 — houserules 성장 규칙). 발견한 모순은 넘기기 전에 해소한다 — 구현 중에 드러나면 설계 반송 왕복이 된다. **`design-tokens.json`이 있으면** 세 회계를 각각 대조한다 — ① `arbitraryValues`(=`--shadow-` 접두 그림자 토큰명·kind=shadow)의 모든 항목이 `app_shadow` 라우팅에 빠짐없이 귀착됐는지(추출 `arbitraryValues` 수 = `app_shadow` 라우팅 항목 수), ② 비도메인 `typography`·`spacing`·`borderRadius` 버킷의 치수 항목이 크기 정형 목록에 빠짐없이 들어갔는지(비도메인 `typography`+`spacing`+`borderRadius` 토큰 수 = 크기 목록 항목 수), ③ `icons[]`의 모든 항목이 ui_extension 아이콘 매핑(또는 `unmappedIcons` 폴백)에 빠짐없이 귀착됐는지(`icons[]` 수 = ui_extension 매핑 항목 수 — `.dc.html` 시안이면 `icons[]`는 extract_dc가 RMW로 주입한 수집분이다) — 어느 쪽에도 안 든 빈칸은 coder가 흘릴 자리다. **`has_design_images`이면** `asset-manifest` 항목이 이미지 연결 정형 목록에 빠짐없이 들어갔는지도 대조한다(manifest 항목 수 = 목록 항목 수) — 빈칸은 coder가 흘릴 이미지다. 독립 리뷰어는 이 자기점검을 대체하지 않는다(그들은 lens별 타당성을, 자기점검은 절 간 일관성을 본다).
