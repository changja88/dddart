...
- (있으면) `design-ref/` 경로 — 화면 디자인(이미지·HTML 시안·토큰 메모). "무엇처럼 보이나"의 단일 근거다. 디자인 도구가 생성한 **코드 자체의 직수입은 금지**하되, 그 안의 **레이아웃 구조(요소 배치·순서·아이콘 처리·정렬·열 구성)와 토큰(색·타이포·spacing)은 구조 충실하게 추출**해 명세에 반영한다 — 요소 유무만이 아니라 **배치·구성까지**(픽셀 아닌 구조 일치). HTML 시안이 있으면 그 **텍스트에서 토큰을 직접 읽는다**(손요약으로 값을 잃지 않는다 — 메모는 보조). **화면 시안과 디자인시스템 토큰이 같은 출처(프로젝트)인지 확인**한다 — 다른 출처의 팔레트를 섞지 않는다.
- 설계 명세를 저장할 경로.
