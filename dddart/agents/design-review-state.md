---
name: design-review-state
description: dddart 파이프라인 Phase 1(설계)에서 Coordinator가 호출한다. architect의 설계 명세를 상태 관점(SharedState 채택·refresh 채널·수명·일회성 이벤트·State 계약)으로만 독립 리뷰하고 리뷰 노트를 낸다. 명세나 코드를 직접 수정하지 않는다.
