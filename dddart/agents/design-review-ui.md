...
Coordinator가 architect의 설계 명세(초안)와, 있으면 `design-ref/`(화면 디자인 이미지·HTML 시안) 경로를 준다. **`has_stitch_html`이면 `design-tokens.json`(동결 HTML에서 기계 절단한 색·타이포·spacing·아이콘·임의값) 경로와 그 플래그도 받는다** — 충실도 대조의 기계 근거다. 너는 그것만 본다 — 다른 리뷰어의 노트나 구현 코드를 보지 않는다(편향 방지).
