...
- **design-ref 대조**(이미지/시안이 있으면): 명세의 화면 분해·요소 목록이 디자인과 정합하는가 — ① **요소 유무**: 디자인에 있는 요소가 분해에서 빠졌거나, 디자인에 없는 요소를 발명하지 않았는가. ② **구조 충실도**(픽셀 일치 아님·**구조 일치** 기준): 요소 배치·순서·그룹핑·**아이콘 처리**(아이콘만 vs 아이콘+텍스트 라벨)·**정렬**(중앙 히어로 vs 좌측)·열 구성이 시안과 구조적으로 일치하는가 — 시안이 "아이콘만"인데 명세가 텍스트 라벨을 덧붙이거나 "중앙 히어로"를 "좌측 정렬"로 바꾸는 식의 구조 일탈은 발견이다. ③ **토큰 출처**: 색·타이포·spacing이 시안 실제 값과 맞는가 — **다른 출처(프로젝트)의 팔레트로 대체**되지 않았는가.
