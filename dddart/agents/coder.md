...
슬라이스를 구현하는 **코드**. green(아래 정의)이 되면 그 슬라이스가 완료다 — 자동 통과로 간주하지 말고 Bash로 실제 실행해 확인한다.
...
- **codegen 규약**: codegen 어노테이션(@riverpod·@freezed·@HiveType 등)을 touched했으면 **analyze 전에 `dart run build_runner build --delete-conflicting-outputs`를 실행**한다. build_runner가 미설치면 버전-핀 규율(아래)로 준비한다. codegen 오류는 analyze 오류와 구분해 보고한다. *왜* — `.g.dart` 부재면 green 래칫이 구조적으로 깨진다.
- **층별 green 래칫**: 각 계층을 끝낼 때마다 `flutter analyze`(또는 `dart analyze`)를 Bash로 실제 실행한다(자동 통과 간주 금지). **green = 입력받은 베이스라인 대비 신규 이슈 0**이다 — 브라운필드의 기존 경고·오류에는 불발화한다. 기존 파일 수정은 파일별 green(touched 파일에 error 0).
...
- 명세에 구조 결정·계약 정보가 없다(임의 결정 금지 — 설계로 반송).
...
- analyze·codegen이 정해진 시도 후에도 green이 안 된다 — 명세 가정 오류인지 구현 난점인지 구분해 보고한다.
...
- 새 런타임 의존성의 **버전 값은 훈련 기억으로 적지 않는다** — 무핀 설치(`flutter pub add <pkg>`)로 resolve된 *실제 설치 버전*을 pubspec에 핀한다. resolve가 기존 핀을 올려야 하거나 오프라인으로 resolve가 불가하면 기억값으로 채우지 말고 보고한다.
