...
- (있으면) `design-ref/` — **화면 레이아웃 형상의 단일 근거.** 배치·축(세로/가로)·그룹핑·정렬·간격은 명세가 아니라 동결된 화면 JSX 시안(`design-ref/screens/*Screen.jsx`)이 정하고 너는 Flutter로 충실 재현한다(implementation-flutter §9). *시각 근거*에 그치지 않는다. **JSX는 React라 직수입하지 않는다 — 형상(배치·축)만 읽어 위젯은 Flutter로 새로 쓴다**(React→Flutter 직역 금지·dddart.md 경계 규율). 토큰·아이콘·이미지는 명세와 `asset-manifest.json`이 이미 절단해 준다.
