...
- **표적은 추출 트랙**: `design-tokens.json`의 `arbitraryValues`(예 `text-[Npx]`)와 비도메인 `typography` 항목이다. 시안의 plain Tailwind 치수 유틸(`w-24`·`text-4xl` 등)도 추출이 `w-[96px]`·`text-[36px]`로 px 정규화해 `arbitraryValues`에 합류시키니, 대괄호 리터럴만 표적으로 좁히지 말고 *정규화된 치수 토큰까지* 전수한다(미세 간격 `gap-`·`p-`·`m-`은 §7 `app_spacing` 소관이라 추출되지 않는다 — 표적 아님). 도메인에 묶인 typography(본문·강조 텍스트 등)는 ui_extension(§5)·토큰이 이미 담당하니 제외 — 시각 눈대중("큰 요소")이 아니라 *기계 추출된 토큰 목록*을 1건씩 본다. 추출된 *typography* 크기(`text-[Npx]`)도 전수 표적이되 그 *적용*은 직접 인용이 아니라 `app_typography` 토큰 정의다(아래 '발명이 아니라 인용').
