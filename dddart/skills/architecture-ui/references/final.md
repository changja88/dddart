...
- **내비 인자(path-param) 가공은 VO/VM 소유 — 뷰 onTap 인라인 직렬화 금지(codex식 엄격 분리)**: 라우트 path-param으로 넘길 도메인 값의 직렬화(예 `DateTime`→`YYYY-MM-DD` 라우트 키)는 **VO의 변환 메서드**(`ForecastDate.pathValue` 류)나 VM이 소유한다. **기본형은 뷰 onTap이 도메인 값을 그대로 VM 전환 메서드에 위임**(`vm.openDetail(forecast.date)` — VM이 VO 직렬화 + navigator 호출을 소유)하는 것이다. 뷰가 navigator를 직접 부를 때도 **VO가 노출한 키나 상수만** 넘긴다 — onTap 콜백 안에서 `DateFormatUtil.toIsoDate(...)` 같은 직렬화를 직접 수행하지 않는다. *왜* — path-param 직렬화는 사람이 읽는 **표시 포맷**(architecture-ddd §5 표현단 변환)이 아니라 **라우트 계약 키 가공**이라, 규약 정합을 도메인 값(VO)이 소유해야 화면이 직렬화 규약을 떠안지 않는다(뷰 수동성). 표시용 날짜 포맷(`6/13 (금)`)은 그대로 표현단이 수행한다 — 둘은 다른 변환이다.
