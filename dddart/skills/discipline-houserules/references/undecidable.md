...
**절차** (feedback-031 — area 규칙 본문은 houserules final.md §1 area 핵심 사실): 동일 접두(BC명의 첫 `_` 앞 토큰 — `_` 없으면 BC명 전체) BC가 2개 이상이면 그 접두는 area 후보다. 그러나 접두가 **역할·서브도메인 축**(예: `driver_*`/`rider_*` — 앱 셸이 갈리는 사용자 역할)인지 **도메인 어휘**(예: `chat`/`chat_request` — 한 기능 군집의 개념 연쇄)인지는 기계가 못 가른다. **판정은 사용자가 한다**: Coordinator가 G0 배너 직전 결정적으로 감지해(기존 BC 목록 조회 + `.dddart/config.json`의 `area_prefixes` 기판정 스킵) G0 배너에 질문으로 표면화하고, 판정을 `area_prefixes`에 `"<접두>": "area" | "not-area"`로 영속한다 — `not-area`는 자동 감지의 재질문만 억제하고 사용자 명시 선언은 항상 우선한다. 에이전트는 area를 자기 판단으로 발명하지 않는다.

**신호**: area 쪽 — 접두 그룹별로 root 라우팅에서 별도 셸이 분기 · 접두가 사용자 역할/앱 모드와 일치. 도메인 어휘 쪽 — 접두 BC 하나가 다른 접두 BC의 UseCase를 상시 호출(한 기능의 개념 연쇄) · 접두가 명사구의 수식어(`order`/`order_return`).
