...
dddart가 만드는 코드에 한정된 집안 규칙이다. **표준 파일트리·명명·import의 사실은 `references/final.md`가 단일 출처**이고, 이 본문은 그 사실을 쓰는 결정 절차다. 보편 클린코드는 discipline-cleancode, 계층 동작 규율은 architecture 4종(ddd·ui·state·data), 문법 표기는 implementation 3종 소유.

## §1 파일트리 결정 순서

새 코드를 배치할 때 위에서부터, 결론이 나면 멈춘다.

1. **기존 프로젝트의 확립된 규약을 우선한다(일관성 최우선).** 단 "존중"은 확립된 규약을 따르는 것이지 미조직 평면 상태의 답습이 아니다. dddart는 기존 코드 수정을 요구하지 않는다 — 표준은 **새로 만드는 코드부터** 적용한다(final.md §7).
2. **신규 단위(BC·애그리거트·개념 폴더·화면)는 표준 트리를 적용한다** — `references/final.md` §1을 반드시 읽는다. 생략·축소 불가 골격(§3 정신, YAGNI로 접을 수 없다):
   ...
   - `application/` 직속은 BC 폴더만. BC 루트 직속은 `<bc>_router.dart`·`<bc>_navigator.dart` 둘만.
   - `lib/root/`는 자체 골격(역할 4폴더, scaffold만 삼총사), design_system은 foundation 7토큰+부품군 골격.
...
4. **의미 판별 17종**(view/section, BC 어휘, 살아있는 상태, 두 번째 개념 등)은 `references/undecidable.md`의 절차·배정을 따른다 — 1차 결정자와 검증자가 같은 파일을 본다.
5. **한 프로젝트 안에서 레이아웃을 혼용하지 않는다.**
...
- **기존 규약 vs 표준**: 확립된 기존 규약이 있으면 §1-1대로 기존이 이긴다 — 표준을 그 위에 강제하지 않는다. 신규 단위 내부는 표준이다.
...
- 4계층·종류 폴더 생략, domain_layer 평면(애그리거트 폴더 없음), `application/` 직속에 BC 아닌 파일.
...
- domain_layer에 `package:flutter`, VM·UseCase·State에 design_system import(presentation·BC 루트만 허용), 타 BC의 Repo·box·VM·SharedState 직접 접근(4채널 밖 — final.md §5).
...
| 의미 판별 17종 절차·배정 | [`references/undecidable.md`](references/undecidable.md) |
