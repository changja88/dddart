...
# ---------- F22~F25: application/<area>/<bc> opt-in 그루핑 (feedback-031)
# area = 순수 시각 네임스페이스 — 적극 증명될 때만(직속 파일 0·직속 전부 BC꼴),
# 그 외 전부 기존 s[1]=BC 보수 폴백. 어휘는 중립(fleet/driver_trip/rider_trip — 과적합 가드).

mkbc() { # mkbc <proj> <bc경로(lib-상대)> — 골격 완비 BC 생성(ST4 통과형)
  local root="$1" bcpath="$2" bc; bc="$(basename "$2")"
  local base="$root/lib/$bcpath"
  mkdir -p "$base"; touch "$base/analysis_options.yaml"
  local k
  for k in use_case view_model state shared_state service; do
    mkdir -p "$base/application_layer/$k"; touch "$base/application_layer/$k/.gitkeep"; done
  for k in data_source repository service; do
    mkdir -p "$base/infra_layer/$k"; touch "$base/infra_layer/$k/.gitkeep"; done
  for k in view section widget ui_extension; do
    mkdir -p "$base/presentation_layer/$k"; touch "$base/presentation_layer/$k/.gitkeep"; done
  for k in entity value_object enum domain_service specification; do
    mkdir -p "$base/domain_layer/$bc/$k"; touch "$base/domain_layer/$bc/$k/.gitkeep"; done
  echo "class Aggregate {}" > "$base/domain_layer/$bc/$bc.dart"
}

# ---------- F22: area 하위 신규 BC — 골격 완비·test/ area 미러에서 ST4·TG1 불발화
P="$T/f22"; BASE=$(mkproj "$P")
mkbc "$P" "application/fleet/driver_trip"
mkdir -p "$P/test/application/fleet/driver_trip/application_layer"
echo "void main() {}" > "$P/test/application/fleet/driver_trip/application_layer/driver_trip_vm_test.dart"
OUT=$(run_backstop "$P" --diff-base "$BASE" --only st,tg); E=$?
assert "F22 area BC 골격 완비 — ST/TG 불발화" 0 - 'ST4\|TG1\|ST3\|ST2\|ST1' "$E" "$OUT"

# ---------- F22b: area 하위 미완비 BC — ST4가 BC 단위로 발화(area를 BC로 오인하지 않음)
P="$T/f22b"; BASE=$(mkproj "$P")
mkbc "$P" "application/fleet/driver_trip"
mkdir -p "$P/lib/application/fleet/rider_trip/presentation_layer/view" # BC꼴이되 골격 미완비
OUT=$(run_backstop "$P" --diff-base "$BASE" --only st); E=$?
assert "F22b area 하위 미완비 BC — ST4 발화" 2 'ST4' - "$E" "$OUT"
assert "F22b ST4가 BC명(rider_trip)을 지목" 2 'rider_trip' 'fleet\` 골격' "$E" "$OUT"

# ---------- F23: area 교차 BC import — IM5 4채널(비채널 발화·use_case 채널 통과)
P="$T/f23"; BASE=$(mkproj "$P")
mkdir -p "$P/lib/application/fleet/driver_trip/application_layer/view_model"
mkdir -p "$P/lib/application/fleet/rider_trip/infra_layer/repository"
mkdir -p "$P/lib/application/fleet/rider_trip/application_layer/use_case"
...
mkdir -p "$P/lib/application/legacy_pay/viewmodel"
mkdir -p "$P/lib/application/billing/application_layer/use_case"
cat > "$P/lib/application/legacy_pay/viewmodel/pay_vm.dart" <<'EOF'
import '../../billing/application_layer/use_case/charge_use_case.dart';
class PayVm {}
EOF
cat > "$P/lib/application/billing/application_layer/use_case/charge_use_case.dart" <<'EOF'
import '../../../legacy_pay/viewmodel/pay_vm.dart';
class ChargeUseCase {}
EOF
OUT=$(run_backstop "$P" --diff-base "$BASE" --only cy); E=$?
