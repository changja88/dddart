...
- BC 도메인 데이터의 캐시는 그 BC `data_source/local_storage/`(접근·스키마·배선 3파일), common/local_database는 엔진·전역 데이터만 — 타 BC box 직접 접근 금지, UseCase 호출로 (§5)
- hive는 local_storage 3파일 — 접근 `_local_data_source`·스키마 `<개념>_box.dart`(@HiveType 모델 1개)·배선 `<bc>_hive_adapters.dart`(등록 함수만 — root_initializer가 import하는 유일한 BC infra 파일) (§5)
