...
Coordinator가 architect의 설계 명세(초안)와, 있으면 `design-ref/`(화면 디자인 이미지·HTML 시안·`notes.md` 메모) 경로를 준다 — Codex는 이미지 판독이 비보장이나 **HTML 시안은 텍스트라 판독 가능**하다(이미지만이면 메모 우선). 너는 그것만 본다 — 다른 리뷰어의 노트나 구현 코드를 보지 않는다(편향 방지).
...
- **design-ref 대조**(이미지·HTML 시안·`notes.md`가 있으면): 명세의 화면 분해·요소 목록이 디자인과 정합하는가 — ① **요소 유무**: 디자인에 있는 요소가 빠졌거나, 없는 요소를 발명하지 않았는가. ② **구조 충실도**(픽셀 아닌 **구조 일치**): 요소 배치·순서·그룹핑·**아이콘 처리**(아이콘만 vs 아이콘+텍스트 라벨)·**정렬**(중앙 히어로 vs 좌측)·열 구성이 시안과 구조적으로 일치하는가. ③ **토큰 출처**: 색·타이포·spacing이 시안 실제 값과 맞는가 — 다른 출처(프로젝트) 팔레트로 대체되지 않았는가. (HTML 시안 있으면 텍스트 토큰으로 대조·이미지만이면 notes 우선.)
