...
Coordinator가 architect의 설계 명세(초안)와, 있으면 `design-ref/`(화면 디자인 이미지·`notes.md` 텍스트 메모) 경로를 준다 — Codex에서는 이미지 판독이 비보장이라 메모가 시각 근거 우선이다. 너는 그것만 본다 — 다른 리뷰어의 노트나 구현 코드를 보지 않는다(편향 방지).
