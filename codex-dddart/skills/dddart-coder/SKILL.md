...
- (있으면) `design-ref/` — 화면 구현 시 시각 근거. **레이아웃 구조(배치·순서·아이콘 처리·정렬)와 토큰을 구조 충실하게 재현**한다(픽셀 아닌 구조 일치) — 도구 생성 코드 직수입은 금지하되 구조·토큰은 따른다. Codex는 **HTML 시안이 있으면 그 텍스트 우선**(이미지만이면 notes 보조).
