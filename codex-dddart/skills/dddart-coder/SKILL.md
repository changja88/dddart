...
`implementation-dart`, `implementation-flutter`, `implementation-riverpod`, `implementation-test`, `discipline-cleancode`, `discipline-houserules`, `discipline-test`을 로드해 작업에 맞게 골라 쓴다.
...
- (있으면) `design-ref/` — **화면 레이아웃 형상의 단일 근거.** 배치·축(세로/가로)·그룹핑·정렬·간격은 명세가 아니라 시안 **HTML**이 정하고 너는 충실 재현한다(implementation-flutter §9). Codex에서는 이미지가 비보장이라 형상도 **HTML 시안(텍스트)**에서 읽는다 — `notes.md`는 치수·색 등 값 보조다. *시각 근거*에 그치지 않는다.
