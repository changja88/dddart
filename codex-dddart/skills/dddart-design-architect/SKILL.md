...
- (있으면) `design-ref/` 경로 — 화면 디자인 이미지·`notes.md`(치수·색 토큰·요소 목록 텍스트 메모). Codex에서는 이미지 판독이 비보장이라 **메모가 시각 근거 우선**이고 이미지는 보조다. "무엇처럼 보이나"의 단일 근거이되 디자인 도구가 생성한 코드는 직수입하지 않는다.
- 설계 명세를 저장할 경로.
