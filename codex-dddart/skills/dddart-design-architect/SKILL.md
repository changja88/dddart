...
- **화면(ui)**: 화면 분해 — view(라우트 단위)·section(VM이 필요한 화면 전속 하위)·widget(수동 부품)의 판별과 *왜*, design_system 재사용 항목(조사 결과 — 새 토큰·공용 위젯이 필요하면 그 결정), 내비게이션 흐름(GoRoute 경로·branch 소속·진입/복귀), view 수동성(판단·가공이 VM으로 갔는지). **`design-tokens.json`이 있으면 그 색·spacing·아이콘을 명세 화면 절에 박는다** — `colors`의 신규 색은 design_system foundation 토큰 추가로 결정하고, 아이콘은 도메인 enum→`Icons.*` 매핑을 ui_extension에 둔다(architecture-ui §5 — `name`/`fill`/`flutter` 후보 인용·`unmappedIcons`는 가장 가까운 `Icons.*` + design-ref 충실도 확인). **`layout-ir.json`이 있으면(`has_layout_ir`) 화면 골격을 명세 화면 절에 *강제로* 박는다** — 각 화면을 **area 어휘 트리**로 옮긴다(raw JSON을 붙이지 말고 layout-ir가 가리키는 닫힌 어휘를 코드 근접 트리로 직렬화): 예 `screen → [ appbar(slots: icon,text), image(src,alt), section "Featured"(repeat-group: unit[...]), section "Categories"(...), bottomnav(slots: button×N) ]`. **L1**(골격 — area 존재·종류·순서: appbar/image/section/bottomnav)과 **L2**(섹션 내부 구성)를 박되 — ⓐ 닫힌 어휘는 layout-ir 스키마 토큰만(area `appbar/image/section/bottomnav`·block `block/repeat-group`·slot `text/icon/image/button/group`·width `fixed/flex/auto`) ⓑ 반복은 enumeration이 아니라 construct(`repeat-group: unit`) ⓒ 기하는 상대만(순서·중첩·flex — 픽셀 절대 금지) ⓓ few-shot 예시는 1~2개. **위젯 클래스명(`AppBar`·`BottomNavigationBar` 등)은 박지 않는다** — area 어휘는 *무엇을*(영역 구성)이고, 그것을 *어떤 위젯으로* 짤지(view/section/widget 분해·MVVM)는 coder가 implementation-flutter 규범으로 자율 결정한다(직교 보존 — 엔진이 dddart 규약대로 위젯을 산출). **너는 area 골격을 JSON으로 추론·출력하지 않는다**(layout-ir는 읽는 입력·area 트리는 산문 명세 — format tax 회피).
