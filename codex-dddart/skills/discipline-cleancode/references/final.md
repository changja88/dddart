...
1. 처음에는 길고 복잡해도 좋다
2. 다듬고 또 다듬는다
3. 다듬는 와중에도 항상 단위 테스트는 통과한다 — dddart 번역: **analyze green 래칫 + 행위 검증 테스트가 그 자리다**(층별 신규 이슈 0 유지·green=`flutter test`; 단 레거시 전체를 감싸는 특성화 테스트는 비강제 — §16·테스트 규율은 discipline-test·implementation-test)
