...
> **시나리오 좌표**: S1 = 신규 BC 공지(~13파일). 두 안의 갈림은 *중대형 기능*에서만 발동(`slice-simulation.md §3` "갈림의 본질") — 그래서 1차 실측 대상. S2(기존 BC 확장 7+3)·S3(수정 모드 5+2)는 보조 시나리오(필요 시 형제 파일 SCENARIO-S2/S3).
