...
> **역할 경계**(오판 방지): **§1·§2(새 폴더 생성·플러그인 재동기)는 *준비*다 — "라이브런 준비" 요청 시 오케스트레이션 에이전트가 직접 실행**한다(사용자에게 넘기지 않는다). **§3·§4(프롬프트·게이트)는 *런*이라 사용자 드라이브** — 평가 대상이므로 사용자가 *새 세션*에서 `/dddart`를 구동한다(런 내부 dddart 파이프라인이 green 빌드까지). **§5 채점은 런 종료 후 에이전트가 지원**하고(FID 게이트 포함 — `fid-gate.sh`로 시안↔코드 구조 대조·2026-06-19 활성), 실기기 런타임(`flutter run`·스크린샷)만 사용자다. 입력 정본 `SCENARIO-WEATHER.md` · 환경 `TEST-ENV.md` · 채점 `rubric/`.
