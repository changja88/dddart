...
> 사용자 드라이브. 에이전트는 green 빌드까지·실기기 런타임은 사용자. 입력 정본 `SCENARIO-WEATHER.md` · 환경 `TEST-ENV.md` · 채점 `rubric/`.
