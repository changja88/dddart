...
1. **RUBRIC 동결**: `RUBRIC.md`의 차원·PASS/FAIL·레인·치명 18 목록(+**FID-L1·L2 조건부** — §H 활성 조건 미충족 시 비활성·치명 집계 미산입)(채점 중 추가·삭제·기준 변경 금지 — §5).
...
> 판정원 = **시안 layout-ir(`dddart/scripts/extract_layout.dart`) vs 생성 코드 렌더 덤프(`tools/dump_probe.dart.txt`+`tools/dump_to_ir.dart`)**의 결정론 대조(`tools/compare_layout.dart`·§3 평탄화). **결정 레인 전담**(구조 대조는 의미 grader 무관). **step 2a·2b 핵심 입증 완료**(8차 실측: L1 image/bottomnav 갭 결정론 포착·weekly card/metrics repeat 등가 흡수). **게이트 활성 선결 1건**(hero 인접 text 흡수 false regression은 `_collapse`로 보정 완료·2026-06-19): 표준 pump 진입점 규약화(코퍼스·승인). 그 전까지 비활성·A1 위임. `RUBRIC.md §H` 정합.
...
> **FID 집행 주의**: ① 도구 — 시안 파서=`dddart/scripts/extract_layout.dart`(**별도**·토큰은 `extract_design.dart`·구현 step 2a) / 대조=`tools/compare_layout.dart`(구현 step 2a) / 코드 렌더 덤프=`tools/dump_layout`(**step 2b 미구현**). 렌더 덤프 부재 → 코드 layout-ir 안 나옴 → **게이트 비활성·시각 충실도는 현행 A1(사용자 눈·RUNBOOK) 위임**(결과지 FID ➖·❌ 금지·coordinator가 수기 게이트 흉내 안 냄·blind 보존·`RUBRIC.md §H`). ② 렌더 덤프는 "표준 pump 진입점"(feedback-011 항목 4·코퍼스·step 2b) 전제 — 부재 시 배선 추론. ③ L4(미관·픽셀·아이콘 심볼)는 자동 판정 없음(§2.5 A1).
