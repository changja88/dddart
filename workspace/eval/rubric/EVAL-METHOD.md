...
모든 신규 결과지는 `rubric-metrix.md`의 골격·순서·칼럼을 유지한다. 섹션 순서(어기면 형식 위반): 1.헤더(§6.2) → 2.빌드 게이트(BG·verdict-first) → 3.치명 17(축별·종합 판정·근거 인용; 결정∥의미 분리는 의미적 변종 메타) → 4.차원별 판정(A.S-DDD→B.S-VIEW→C.S-STATE→D.S-DATA→E.S-HR) → 5.TIER-Q 등급 → 5.5 grader 패널 증거(per-grader raw verdict·차원별 κ·비-Claude 오라클 유무·**rubric 사각 신고**·A3/A13) → 6.의미적 변종/백스톱-blind 메타(*측정의 주 산출물*·조정자 노트보다 앞) → 7.발견 로그 → 8.잔여흠 원장 → 9.한 줄 요지(+두 안 비교 시 비교 집계지 부록).
