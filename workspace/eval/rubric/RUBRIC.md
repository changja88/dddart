...
> **명시적 비측정(과대주장 제거)**: baseline 대비 가치 · 미시 가독성·복잡도(유지보수성은 구조 대리까지) · 런타임 성능·보안(후속 트랙) · 명세 내적 품질(후순위) · 파이프라인 프로세스 규율(coder 경량본 사용·리뷰어 권한 경계 등 — `EVAL-METHOD.md` 소관) · **미관/픽셀 충실도**(간격·실제 색·미관·아이콘 *심볼*): AI 채점자가 렌더를 못 봐 구조적으로 못 닫음 → **인간 오라클 위임·자동 채점 비측정(A1·L4)**. *단 시각 충실도 중 **구조 골격(L1)·섹션 구성(L2)·말단 슬롯(L3)**은 layout-ir 결정론 대조로 측정한다(§H FID — 비측정 아님; L1·L2는 게이트·L3는 약신호)*. VW-4/5는 토큰 단일출처·매핑 *거주*만 보지 *시안 일치*를 안 본다(FID가 구조 일치를 봄·직교). 미관·L4·아이콘 심볼은 평가지 밖/위임/후속. 단 기존 App* 시각 토큰 위에 `copyWith`로 typography 시각값(`fontSize`·`height`(행간)·`letterSpacing`)을 리터럴로 덮으면 그 값이 토큰 밖에 거주하므로 VW-4(거주 우회)지 시안 일치/FID-L4가 아니다 — 비-typography size prop(`width`·박스 height·아이콘 size) 추출값 직접 인용·시안 명시값 없는 임의 미관 수치는 그대로 §8 정식/FID-L4.
...
| **VW-4** 시각 토큰 단일 출처 | foundation 토큰만·VM 시각 getter 금지 | ui §7 / HR NM10 | 색·타이포·duration이 App* 토큰(foundation 7파일) 참조, VM/State에 시각 반환 getter·design_system import 0 | 토큰 밖 시각 리터럴(`Color(0x…)`·생 TextStyle·`Colors.*`·생 *시각* Duration[전환·애니메이션 등 연출] — 단 네트워크 timeout·디바운스 등 *비시각* duration과 `Colors.transparent` 류 비브랜드 구조 상수는 제외·architecture-ui §7)·**기존 App* 토큰을 `copyWith`로 받으며 typography 수치 prop(`fontSize`·`TextStyle.height`·`letterSpacing`)을 리터럴 덮기**(`AppTypography.x.copyWith(fontSize:N)` — 값이 토큰 밖 거주·§7 생 TextStyle 금지의 변종·NM10 사각→reviewer; 단 비-typography size prop[`width`·박스 height·아이콘 size] 추출값 직접 인용은 §8 정식·위반 아님) 또는 VM/State 시각 매핑 | 결정+의미 | — |
...
## I. TIER-Q 품질 (카운트 기반 — 치명 아님; `EVAL-METHOD.md` 등급)
...
- 시각 *구조* 충실도: **FID**(L1 골격·L2 섹션·L3 슬롯) · 토큰 거주: **VW-4** · 매핑 거주: **VW-5** · 미관·픽셀·아이콘 *심볼*: **A1 인간**(FID-L4) — FID는 시안 *구조 일치*, VW-4/5는 *거주 위치* — 직교(이중 채점 금지). 단 토큰 위 typography 시각값(`fontSize`·`height`·`letterSpacing`) `copyWith` 리터럴 오버라이드는 값의 단일출처 우회라 VW-4(거주)다 — FID-L4(미관)로 흘리지 않는다(11차 grader 흘림 교정).
