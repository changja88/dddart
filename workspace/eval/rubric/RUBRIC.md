...
| **FC-2** 테스트·메커니즘 비-vacuous | 핵심 로직에 mutation(정렬 역전·조건 반전·status 값) 주입 후 red 확인; 재탭 2단·내비 등 lint 사각 메커니즘은 widget test로 행위 검증 | mutation마다 red·메커니즘 행위 검증됨 | mutation에도 green(헛 테스트)·메커니즘 미검증 | 결정(주입 실행) | ✅ |
