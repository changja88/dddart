...
치명 게이트 18개(+FID-L1·L2 활성 시 20) — 하나라도 FAIL이면 픽스처 전체 FAIL. **빌드 게이트**(컴파일·analyze green)는 dddart가 산출물 테스트가 얕고 codegen 의존이라 신설한 1차 정확성 기질.
...
3. **섹션 순서**(EVAL-METHOD §6.1): 헤더 → 빌드게이트 → 치명18(+FID 활성 시 20) → 차원별(S-DDD→VIEW→STATE→DATA→HR) → TIER-Q → 의미변종/백스톱-blind 메타 → 발견 로그 → 잔여흠 원장 → 한 줄 요지 (+두 안 비교 시 비교 집계지 부록).
