...
> **상태: v3 — 확정 (2026-06-12).** 적대 리뷰 3렌즈(skill-creator·plugin-dev·내부 정합, 22건) 반영 후 검증 워크플로 3렌즈(반영 완전성 pass·상위 정합 5건·자기 모순 6건)까지 반영. 단일 근거: 본설계 §8(절 귀속·lens 경계)·§9(판별 배정)·§10-3(제약), 제1 규약, `workspace/reference/spec.md`(작업장 규약). 리뷰 결정 2건 채택: ① 규약 §6 common → houserules 귀속(본설계 §8 동시 개정) ② Wave별 경량 관통 테스트 도입.
...
2. **Wave 1 완료 후 사용자 검토** — 양식·분량 기준 확정. 이때 절 단위 부분 적재 지침(TOC·헤더 규약)의 실효도 함께 확인.
