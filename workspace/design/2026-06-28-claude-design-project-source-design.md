...
4. **순서(MF-1 귀결·T2 구현이 표면화)**: 동결·추출(`extract_design`→`extract_dc`)은 게이트 자료(`screen-meta.json`·렌더)를 만들려고 확인 *전*에 수행한다. 단 **승인 전 *진행*(Phase 1·`has_design_screen` 확정) 금지** — 거부 시 동결 design-ref를 폐기하고 다른 화면/자체설계로 간다.
