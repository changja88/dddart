...
- 상태: **확정(2026-06-12) — 적대 리뷰 5렌즈 통과·반영 완료, 사용자 확정** (§6 슬라이스 분할만 2단계 실측 후 최종화하는 잠정 확정)
- 입력: 제1 규약 `2026-06-11-dddart-file-tree.md`(확정) · dddjango 실물(`/Users/hyun/Desktop/dddjango/dddjango/`) · `2026-06-12-slice-simulation.md` · 적대 리뷰(blocker 11·사용자 결정 4건 — 전부 반영, 리포트는 git 이력 보존)
- 문서 통합(2026-06-12): 에이전트 구성 문서(`pipeline-agent-composition.md`)는 이중 근거 방지를 위해 **§1.1~1.4·§8로 흡수 후 삭제**(git 이력 보존) — 이 문서가 에이전트 구성의 단일 근거다.
...
### 1.1 에이전트 구성 — 커맨드 1 + 에이전트 7종

dddjango와 같은 수. 골격("단일 작성자 → 독립 lens 리뷰 → 명세 집행자 → 독립 감사자")은 플랫폼 중립이라 그대로, lens만 클라이언트로 재편.

| dddjango | dddart | 변화 |
|---|---|---|
| Coordinator 커맨드 | **Coordinator** (`/dddart`) | 동일 + 슬라이스 기계 도출·외부 진실 스냅샷·git 스냅샷·build-state(§1 직접 쓰기 목록) |
| design-architect | **design-architect** | 동일(통합 명세 단일 작성자·리뷰 중재) + design_system·common 재사용 후보 조사 + 판정 소유 라벨링(§5) |
| design-review-ddd | **design-review-ddd** | 점검 클라이언트화: 판정 소유(1곳째부터 domain 기본·강등 — 제1 규약 §3.3), BC 배치, 교차 BC 4채널 선택 적정성 |
| design-review-api | **design-review-data**로 흡수·변형 | 서버는 계약 *생산*, 클라이언트는 *소비* — 가정 계약 명시성(항상)+실계약 대조(스냅샷 있을 때), Either 계약, DataSource 분해, hive 저장·무효화 |
| design-review-db | (위 data로 흡수) | 클라이언트 영속=hive — 서버 db lens의 무게(마이그레이션·인덱스·트랜잭션)가 없어 독립 lens 가치 없음 |
| — | **design-review-ui** (신규) | 관찰 표면이 HTTP가 아니라 화면 — 화면 분해(view·section·widget)·디자인 시스템 재사용 누락·내비게이션 흐름·view 수동성 |
| — | **design-review-state** (신규) | 서버에 없는 축 — SharedState 채택 판단(과채택/누락), refresh 채널, keepAlive/autoDispose 수명, 일회성 이벤트 소비 |
| acceptance-tester | **없음** | 테스트 없음 결정 — 두 기능의 흡수는 §1.3 |
| coder | **coder** (단일) | 명세 집행·반송 규율 동일, TDD 내부 루프 대신 층별+파일별 green 래칫(§6) |
| discipline-reviewer | **discipline-reviewer** | 동일(게이트 직전 읽기 전용) — 결정적 백스톱이 못 보는 의미 변종 전담(분업 dddjango 동형) |

### 1.2 리뷰어 4축 전부 항상 활성 (풀 빌드)

스코프에서 활성 lens를 추론·제안하는 dddjango 방식을 채택하지 않고 **ddd·ui·state·data 4종을 매 풀 빌드 전부 호출**한다. 근거: ① 활성 판단 자체가 기계 판별 불가 — 특히 state(교차 BC 전파 필요)는 스코프가 아니라 설계 중에 드러나는 사실 ② 활성 오판은 잡을 자 없는 침묵 사각 — 리뷰어의 "누락 자체를 발견으로" 규칙은 그 lens가 활성일 때만 발화 ③ 비용 비대칭(미활성 오판 ≫ 병렬 리뷰 1회·"해당 없음"도 검증) + 절약 효과 미미 ④ 비결정성 제거. 각 리뷰어는 관심사가 없으면 **"해당 없음 + 근거 한 줄"** 의무(침묵·생략 금지). **수정 모드는 예외** — touched 계층은 추론이 아닌 기계 신호이므로 touched-layer 매핑으로 줄인다(§7, 결정 D2).

### 1.3 acceptance-tester 부재의 흡수

| dddjango에서의 기능 | dddart 흡수처 |
|---|---|
| 슬라이스 앵커(1 인수 테스트 ≈ 1 슬라이스) | Coordinator의 규모 함수 기계 도출(§6) |
| Green 판정자(완성의 기계적 정의) | 기계 최대치 = analyze(베이스라인 대비 신규 0)+빌드+백스톱("규약을 안 어겼다") / "요구를 충족했다"는 G2 행위 체크리스트 사용자 눈 확인 |

### 1.4 검토 후 기각·보류한 구성 후보

| 후보 | 판정 | 이유 |
|---|---|---|
| 시각 검증 에이전트(스크린샷 판독) | **보류** | 판독 불안정·검증 비용 최소화 취지와 충돌. G2 스크린샷 *첨부*(§6 — 판독은 사용자)와는 별개 |
| 디자인 시스템 정찰 에이전트 | 기각 | architect 조사 의무로 흡수 + ui 리뷰어가 재사용 누락 점검 |
| ui-coder / logic-coder 분리 | 기각 | 단일 coder가 전 슬라이스를 같은 명세로 집행해야 정합(분리 시 명세 해석 이원화·주입 중복) |
| 백스톱 러너 에이전트 | 기각 | Coordinator가 Bash로 직접(러너 스크립트 1개 — §10) |
...
**lens 경계 — state vs data(한 주제 한 소유자)**: data = 데이터가 앱 바깥(서버·디스크)과 어떻게 오가는가(주로 `infra_layer`) / state = 들어온 데이터가 앱 안에서 화면들 사이에 어떻게 살아 있는가(주로 VM·State·SharedState). 판례 ① **캐싱**: hive 저장 = data, 메모리 keepAlive = state(목적이 같아도 소유자가 다름) ② **에러**: 서버 에러가 오는 모양(Either 계약) = data, 그 에러를 State에 담아 표시·소비하는 방식 = state. state 독립의 실측 근거: HaffHaff refresh_notifier가 8 BC·12 VM에 분포, 파이널 리뷰 막힘이 전부 동적 배선.
