...
- 상태: **확정·구현 완료(2026-06-12)** — 적대 점검 2렌즈 반영 + 사용자 확정(§11 4건 전부 권장대로) + 즉시 구현(사용자 결정 A). 구현·검증 결과는 §12
...
4. **§10-5 선결정 목록 합류**: ⓐ State 파일 없는 VM(`AsyncValue<엔티티>` 직노출) 허용 여부(NM4 연동) ⓑ 컨트롤러 소유 계층(IM12 연동) — 제1 규약 §10-5 ①에 등재 후 **2026-06-12 §10-5 ① 확정: 둘 다 규약 문면대로 최종 확정** — 전 VM freezed State(NM4 유지)·컨트롤러 View 소유(IM12 유지).

## 12. 구현·검증 결과 (2026-06-12 완료 — 사용자 결정 A: 즉시 구현)

- **위치**: `dddart/scripts/` — `backstop.dart`(러너 엔트리)·`extract_contract.dart`·`src/`(common+패밀리 4)·`test/run_fixtures.sh`(픽스처 13종). 전부 Dart 코어만, pub 의존 0, `dart analyze` 이슈 0.
- **픽스처 13/13 통과** — 필수 4종(8단 상대 import 클램핑·porcelain `-uall` 신규 BC·블록 주석 directive·멀티라인 `ref.listen<T>(`) + added 줄 게이트 2단(레거시 위반 불발화→신규 줄 발화)·IM5 4채널 선별·CY1 생성→동결→신규 쌍 발화·extract 전이 폐쇄(공유 parameters·security 보존, 비인용 메서드·스키마 절단, 누락 시 exit 1+근사 후보).
- **HaffHaff 실물 스모크**: `--all` 861파일 0.7초·크래시 0·발견 1,462건 — 분포가 제1 규약 §8 drift 실측과 정합(ST5 200·NM15 195 = domain 평면 / NM2 107 = `_app`·`_bridge`·`_block` 구접미사 / IM5 165 = 교차 BC 위반 / IM13 88·NM10 71). **gated(클린 트리) = 발견 0·exit 0** — 브라운필드 면책 의미론 실증.
- 후속: 파이널 리뷰 문서(`2026-06-12-file-tree-final-review.md`)의 §5는 이 문서로 대체 — §6(기계 판별 불가 17종)은 §10-3 공유 reference로 넘어간 뒤 삭제 후보 확정.
