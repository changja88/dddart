...
│       │
│       ├── infra_layer/
│       │   ├── data_source/                 # 데이터 출처 정의 — 원격(retrofit)은 평면에
│       │   │   └── local_storage/           # 로컬 저장 모듈 — 접근(_local_data_source)+스키마(_box)+배선(_hive_adapters)
...
- **infra_layer — 평면 유지.** 개념 분할 없이 종류 폴더만 (HaffHaff 16 BC 전수에서 동일). 유일 하위층 예외는 `data_source/local_storage/`(로컬 저장 모듈 — §5·architecture-data §5·feedback-032)이며, local_storage 자체는 다시 평면이다.
...
| infra `data_source/local_storage/` (접근) | 개념 | `<개념>_local_data_source.dart` | `<개념>LocalDataSource` |
| infra `data_source/local_storage/` (스키마) | 개념 | `<개념>_box.dart` | `<개념>Box` — @HiveType 저장 전용 모델 1개 |
| infra `data_source/local_storage/` (배선) | BC명 | `<bc>_hive_adapters.dart` | (클래스 0 — `register<Bc>HiveAdapters()` 등록 함수만) |
...
- **root → BC는 자유**(전부 아는 것이 존재 이유 — 4채널 면제). 단 Model 방향 규율은 동일: root도 BC의 **UseCase만** 호출(Repo·box 직행 금지). 유일 예외: `root_initializer` → `data_source/local_storage/<bc>_hive_adapters.dart`(데이터 접근이 아니라 시동 배선 — root가 import할 수 있는 유일한 BC infra 파일).
...
- BC의 hive 저장은 `data_source/local_storage/`에 3파일로 산다 — 접근(`<개념>_local_data_source.dart`)·스키마(`<개념>_box.dart` 파일당 @HiveType 모델 1개)·배선(`<bc>_hive_adapters.dart` — 등록 함수만·클래스 0). 도메인 엔티티에 storage 어노테이션을 붙이지 않는다(상세는 architecture-data §5).
...
- `local_database/`는 로컬 DB **엔진**과 **전역 데이터**(토큰·앱 설정)만 — BC 도메인 데이터의 캐시는 그 BC `infra_layer/data_source/local_storage/`(로컬 2층 상세는 architecture-data §5 — 이름 대비[common `local_database` vs BC `local_storage`]가 2층 구분을 어휘로 고정한다).
...
| BC 도메인 데이터 캐시가 common에 | BC `infra_layer/data_source/local_storage/`의 `_local_data_source.dart` |
