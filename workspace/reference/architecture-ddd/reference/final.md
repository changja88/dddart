...
| field | value |
|---|---|
| purpose | dddart가 생성하는 Flutter 코드의 도메인 모델링 단일 출처 — 간소화 DDD의 채택/비채택 지도, 값 객체·엔티티·애그리거트의 freezed 형태, 판정 소유와 강등, 도메인 서비스 주어 귀속, Specification, UseCase 관문, 빈혈 차단. |
| use when | 도메인 모델(애그리거트·엔티티·VO)을 설계·작성·검수할 때, 판정·계산의 소유자를 정할 때, BC 어휘·경계를 판단할 때, full-DDD 패턴(이벤트·CQRS 등)의 채택 여부 질문이 올 때. |
| exclude/handoff | 파일트리·폴더·명명 사실은 discipline-houserules, VM·State 동작은 architecture-state, Repo 구체·Either·직반환 계약은 architecture-data, 캡슐화·SOLID 일반론은 discipline-cleancode, freezed 표기법은 implementation-dart로 위임. |
| core criteria | dddjango architecture-ddd final(Evans·Vernon·Millett·Cosmic Python 합성본)의 선별 이식 + 제1 규약 §3.2·§3.3(판정 소유·강등·UseCase)·§9 결정·§10-5 ③(2026-06-12 사용자 확정 안 A). 간소화 결정과 충돌하는 절은 "비채택" 명시(§10). |
| source priority | 1 제1 규약(간소화 결정이 이론보다 우선) 2 dddjango final 소스판(external.md — internal [A][B][C]+외부 권위 교차 완료본) 3 Vernon "Effective Aggregate Design"·Evans 파란책(전술 패턴 원전). |
| P1 classification | sufficient — §4의 클라 번역(트랜잭션 부재·서버 진실원천·화면 동기화는 SharedState/재조회)은 규약 §9-15·architecture-state §5에서 도출한 조립이며 P1 표에 근거 기록. §10-5 ③ 3규칙은 2026-06-12 사용자 확정. 강등 판별 상세(spec vs domain_service)는 규약 §3.2 문면 직접 도출. Dart 예제는 원료 Python 예제의 의미 보존 치환. |
...
| 채택 | 형태 |
|---|---|
| 바운디드 컨텍스트·유비쿼터스 언어 | BC = `lib/application/<bc>/`(기능 영역 — area 그루핑 시 `lib/application/<area>/<bc>/`, houserules §1 소유), 같은 개념 같은 철자 (§2) |
...
- **바운디드 컨텍스트(BC)**: 하나의 모델·하나의 언어가 일관되게 통하는 명시적 경계다. 같은 단어도 컨텍스트가 다르면 다른 모델이다 — 마케팅의 '리드'와 영업의 '리드'는 다른 클래스로 산다. dddart에서 BC의 물리 형태는 `lib/application/<bc>/`(기능 영역 — 선택적 area 그루핑 시 `lib/application/<area>/<bc>/`, area는 순수 시각 네임스페이스라 BC 경계·통신에 무영향·houserules §1 소유)이고, BC 간 통신은 4채널만 허용된다(닫힌 열거는 discipline-houserules §5). 화면이 어느 BC에 속하는지·BC 어휘를 "보유"하는지의 판별은 `undecidable.md` §3.
...
- **유비쿼터스 언어**: 도메인 전문가와 코드가 같은 단어를 쓴다 — 코드의 클래스·메서드·파일명이 업무 어휘를 그대로 반영해야 대화와 코드 사이의 번역 비용(과 번역 중 왜곡)이 사라진다. dddart의 집행 형태: **같은 개념은 계층이 달라도 같은 철자**(어순 포함 — discipline-houserules §2), UseCase는 화면이 아니라 도메인 개념 단위 명명(§8). **task가 도메인 표시 라벨을 명시 열거하면**(상태 N종을 정본 문자열로 나열 등) 그 문자열이 권위 어휘다 — 코드가 라벨을 어디 두든(도메인 enum 표시명 getter·ui_extension §5) 그 문자열을 **verbatim 보유**하고, 다른 언어로 왕복 번역(task 언어→영어→task 언어)·task에 없는 라벨 발명·task 라벨 누락을 하지 않는다(번역 중 왜곡의 한 형태). 같은 이유로 **도메인 enum 식별자는 서버 계약 enum 값을 verbatim 따른다** — `@JsonValue`로 받는 서버값과 의미가 어긋나는 이름으로 재명명하지 않는다(계약 어휘 보존).
...
| **마이크로서비스식 BC 분리** (BC별 패키지·배포 분리) | 클라 단일 바이너리 — 배포 경계가 없다 | BC는 `application/<bc>/`(또는 `application/<area>/<bc>/`) 폴더 경계 + 4채널로 충분 |
