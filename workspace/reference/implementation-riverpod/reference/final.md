...
| field | value |
|---|---|
| purpose | dddart가 생성하는 코드의 riverpod 표기 단일 출처 — @riverpod 변종 화이트리스트, keepAlive·retry 표기, ref 규율(mounted 가드 포함), AsyncValue API, View 측 표기, 금지 표면, lint 연동. |
| use when | VM·SharedState·Service·root_vm·handler의 @riverpod 선언을 쓸 때, ref.watch/read/listen을 고를 때, AsyncValue를 다룰 때, 재조회·재시도 표기가 필요할 때. |
| exclude/handoff | 수명 *결정*(어느 변종·언제 keepAlive)은 architecture-state §9, State 계약·에러 2채널은 architecture-state §3·§4, @riverpod 허용 위치 닫힌 열거는 discipline-houserules §4, freezed 표기는 implementation-dart로 위임. |
| core criteria | riverpod 3.x 정식(코어 3.3.2·annotation 4.0.3·generator 4.0.4 — 2026-06-12 pub.dev·changelog 원문 확인) 기준. dddart 결정 2건 반영: 자동 재시도 전역 OFF(2026-06-12 사용자 확정)·실험 기능(Mutations·Offline) 비채택. HaffHaff 표기 자산(dev.17)과 정식의 표기 차이는 사실상 없음(파괴 변경은 dev.12·16에 선행). |
| source priority | 1 공식(pub.dev API 문서·riverpod.dev·GitHub changelog 원문 — external.md에 절별 URL) 2 dddart 결정(retry OFF·비채택) 3 HaffHaff 실물 표기(검증 보조). |
| P1 classification | sufficient — 조사(external.md)가 전 항목 공식 원문 대조. retry OFF 확정으로 조사 미확정 1번(재시도 중 error 빌더 동작)은 소멸. 잔여 미확정: riverpod_lint 3.1.x의 최소 Dart SDK(§10에 단서) — 도입 시 pub.dev 제약 1회 확인. |
