...
| field | value |
|---|---|
| purpose | dddart 테스트의 Flutter 메커니즘 표기 단일 출처 — flutter_test/matcher 어휘·mocktail 더블(코드젠 0)·riverpod 3.x ProviderContainer.test·위젯 펌프 결정성(NoSplash·Timer/Completer)·날짜 주입·네트워크 이미지 목·헬퍼 계약. implementation-flutter §7을 흡수해 riverpod 3.x·mocktail로 갱신. |
| use when | 테스트 코드를 작성할 때 — provider/VM 격리, 위젯 펌프, 더블(mock/fake), 비동기·시간·네트워크 이미지의 결정성 확보. |
| exclude/handoff | 무엇을 테스트할지·오라클·비-vacuity·단언 FORM(구별·순서·위치·탭)은 discipline-test, 테스트 파일 위치(test/=lib 미러·sparse)는 discipline-houserules §1·§3, @riverpod·AsyncValue 런타임 규율은 implementation-riverpod, 위젯 수명·async gap context는 implementation-flutter §6으로 위임. |
| use 기준 | flutter_test/package:test(SDK 번들)·package:matcher(dart.dev)·flutter_riverpod 3.x(ProviderContainer.test·read(.future)+pump·overrideWithBuild)·mocktail(코드젠 0·dev)·fake_async+clock(dart.dev)·mocktail_image_network(조건부) — 2026-06-17 확인. 버전 값은 무핀 설치로 resolve(coder 경계 규율). |
| core criteria | mocktail 채택(이미 도는 build_runner에 목 코드젠 비가산 — 생성 파이프라인 결정성)·riverpod 3.x ProviderContainer.test(자동 dispose)·날짜 주입(시간 의존 테스트 금지·게이트 없음·clock 불필요)·golden/patrol/mockito/mutation 주석-제외. |
| P1 classification | sufficient — 헬퍼 계약(§7)은 weather 예시 이름이며 BC별 환언 대상(계약은 유지). 실제 clock.now()를 쓰는 BC 등장 시 §5 시간 통로 재고(현 5차 코드 DateTime.now() 0회). |
